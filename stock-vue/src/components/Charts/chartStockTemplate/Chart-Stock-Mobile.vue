...
<script setup>
    ...
    const fetchMenu = async () => {
        const code = props.prop_search.stock_code || route.query.code;
    ...
    const dynamicChartWidth = computed(() => {
        const minPerLabel = 10  // px per label
        const labelCount = dateLabelData.value.length
        return Math.max(labelCount * minPerLabel, 400) + 'px'
    })
</script>

<style lang="scss" src="@@/charts/Chart-Stock-Mobile.scss" scoped></style>
