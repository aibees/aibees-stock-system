...
class UserOptions(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'user_balance':               _f(self.user_balance),
            'ratio_trend':                _f(self.ratio_trend),
            'ratio_momentum':             _f(self.ratio_momentum),
            'ratio_volatility':           _f(self.ratio_volatility),
            'ratio_volume':               _f(self.ratio_volume),
            'time_frame':                 self.time_frame,
            'macd_recent_day':            self.macd_recent_day,
            'bb_over_recent_day':         self.bb_over_recent_day,
            'vol_limit':                  self.vol_limit,
            'vol_surge':                  _f(self.vol_surge),
            's1_stop_loss_pct':           _f(self.s1_stop_loss_pct),
            's1_take_profit_pct':         _f(self.s1_take_profit_pct),
            ...
            's1_vol_ma_mult':             _f(self.s1_vol_ma_mult),
            's1_regime_window':           self.s1_regime_window,
            's1_regime_threshold':        _f(self.s1_regime_threshold),
            's1_strict_need_macd_up':     self.s1_strict_need_macd_up,
            's1_loose_need_vol_surge':    self.s1_loose_need_vol_surge,
            's1_surge_relax_mult':        _f(self.s1_surge_relax_mult),
            's1_downtrend_surge_bypass':  self.s1_downtrend_surge_bypass,
            's1_surge_bypass_mult':       _f(self.s1_surge_bypass_mult),
            's1_use_trailing':            self.s1_use_trailing,
            's1_trail_basis':             self.s1_trail_basis,
            's1_trail_activate_pct':      _f(self.s1_trail_activate_pct),
            's1_k_trail_atr':             _f(self.s1_k_trail_atr),
            's1_trail_floor_pct':         _f(self.s1_trail_floor_pct),
            's1_time_stop_extend':        self.s1_time_stop_extend,
            's1_time_stop_band':          _f(self.s1_time_stop_band),
