...
from stock_shared.models.userOptionM2 import UserOptionM2

logging.basicConfig(level=logging.ERROR)


class UserOptionM2Dao(BaseDao):
    model = UserOptionM2

    def __init__(self):
        self.__name__ = "UserOptionM2Dao"

    def select_by_user(self, session, user_id: int) -> dict:
        """user_id 의 M2 옵션. 행이 없으면 빈 dict."""
        stmt = select(UserOptionM2).where(UserOptionM2.user_id == user_id)
        row = session.execute(stmt).scalars().first()
        return row.to_dict() if row else {}

    def select_prefixed(self, session, user_id: int, prefix: str = "s2_") -> dict:
        ...
        if not row:
            return {}
        return {f"{prefix}{k}": row.get(k) for k in UserOptionM2.PARAM_KEYS}
    ...
    def upsert(self, session, user_id: int, values: dict) -> None:
        ...
        data.update({k: v for k, v in values.items()
                     if k in UserOptionM2.PARAM_KEYS})
        stmt = mysql_insert(UserOptionM2).values(**data)
