...
전략 ↔ 운용모드(user_trade_mode.active_mode) 매핑. 운용모드는 M1·M2 두 가지뿐이다:
  kospi1      KospiStrategy1  M1  추천매수      (EMA/ATR/OBV. 실전 worker 현행 전략)
  kospi2      KospiStrategy2  M2  ETF 교대      (30분봉. 전략 로직 구현됨, 실전 executor 는 스켈레톤)

  · 두 클래스 모두 StockStrategy 를 직접 상속한다(상호 상속 없음).
  · 클래스 번호 = 모드 번호 = 파일 번호 로 셋이 일치한다.
  · 종목 단위 지정가 매도는 별도 운용모드가 아니라 모드 무관 공용 기능인
    app/trade_worker/sell_executor.py 의 "매도 수기등록"이다.
    자세한 배경은 docs_worker_mode_runtime_spec.md §6·§11.
