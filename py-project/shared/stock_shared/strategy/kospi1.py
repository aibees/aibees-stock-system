"""
kospi1.py — M1 (단일 종목 고정) 전략 골격.

운용모드 M1: 사용자가 지정한 종목 1개만 매매한다.
  · 매수 진입 규칙(entry_rule)
      IMMEDIATE : 장 시작 즉시 전량 매수
      SIGNAL    : 매수 필터 충족 시 매수
  · 매도 판정은 이 클래스가 자체적으로 갖는다(KospiStrategy0 상속하지 않음).
...
class KospiStrategy1(StockStrategy):
    ...
    def configure(self, user_info: UserOptionMeta) -> None:
        ...
        raise NotImplementedError('KospiStrategy1.configure 미구현')

    # ── 매매 판정 ─────────────────────────────────────────────────
    def get_action(self, trade_data: list[dict], user_info: UserOptionMeta) -> Action:
        raise NotImplementedError('KospiStrategy1.get_action 미구현')
    ...
    def get_action_with_prev(self, position_type: str, prev_info: UserCoinInfo,
        ...
        raise NotImplementedError('KospiStrategy1.get_action_with_prev 미구현')

    def get_action_in_watch(self, prev_info: UserCoinInfo, coin_info: UserCoinInfo,
                            user_info: UserOptionMeta) -> dict:
        """미보유 상태 — 매수 여부 판정."""
        raise NotImplementedError('KospiStrategy1.get_action_in_watch 미구현')

    def get_action_in_active(self, prev_info: UserCoinInfo, coin_info: UserCoinInfo,
                             user_info: UserOptionMeta) -> dict:
        """보유 상태 — 매도 여부 판정."""
        raise NotImplementedError('KospiStrategy1.get_action_in_active 미구현')
