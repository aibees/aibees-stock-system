"""
kospi4.py — M4 (지정가 감시) 전략 골격.

운용모드 M4: 사용자가 등록한 지정가 세트(user_option_m4)로만 매매한다.
  · buy_price  이하 도달 → 매수
  · sell_price 이상 도달 → 매도
...
class KospiStrategy4(StockStrategy):
    """M4 : 지정가 감시 매매."""
    ...
    def configure(self, user_info: UserOptionMeta) -> None:
        """user_option_m4 값을 인스턴스 파라미터에 반영."""
        raise NotImplementedError('KospiStrategy4.configure 미구현')

    # ── 매매 판정 ─────────────────────────────────────────────────
    def get_action(self, trade_data: list[dict], user_info: UserOptionMeta) -> Action:
        raise NotImplementedError('KospiStrategy4.get_action 미구현')
    ...
    def get_action_with_prev(self, position_type: str, prev_info: UserCoinInfo,
        ...
        raise NotImplementedError('KospiStrategy4.get_action_with_prev 미구현')
    ...
    def get_action_in_watch(self, prev_info: UserCoinInfo, coin_info: UserCoinInfo,
        ...
        raise NotImplementedError('KospiStrategy4.get_action_in_watch 미구현')

    def get_action_in_active(self, prev_info: UserCoinInfo, coin_info: UserCoinInfo,
                             user_info: UserOptionMeta) -> dict:
        """BOUGHT — 현재가 >= sell_price(또는 <= stop_price) 판정."""
        raise NotImplementedError('KospiStrategy4.get_action_in_active 미구현')
